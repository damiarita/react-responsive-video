...
export default({videoProps, sizes, show}:Props)=>{
    ...
    return(
        <video {...(Object.assign({},videoProps, styles))} height={selectedSize?.height} width={selectedSize?.width}>
            {(selectedSize===undefined || selectedSize.videoSources.length===0)?undefined:selectedSize.videoSources.map(({url, format}, sourceIndex)=>(<source key={selectedSize.mediaQuery+'-'+sourceIndex.toString()} src={url} type={format} />))}
        </video>
    )
}